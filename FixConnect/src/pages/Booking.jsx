  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.booking?.paymentUrl && (formData.paymentMethod === 'PayMongo' || formData.paymentMethod === 'GCash' || formData.paymentMethod === 'Maya' || formData.paymentMethod === 'Credit / Debit')) {
        ...
        setStatus({ type: 'success', message: 'Booking searching! Redirecting to Tracker...' });
        setTimeout(() => {
        ...
        <form onSubmit={handleSubmit}>
          ...
          {!paymentLink ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <Button type="button" onClick={() => navigate(`/booking/${bookingId}`)} style={{ background: 'var(--bg-card)', border: '1px solid var(--primary-color)', color: 'var(--primary-color)', width: '100%' }}>
                Skip Payment & View Tracker
              </Button>
            </div>
          )}
        </form>
